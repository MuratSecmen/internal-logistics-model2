...
#  Gurobi model (eq_3 ... eq_31 + z-based addons)
# ============================== #
import pandas as pd
...
# ---------- Genel Ayarlar ----------
OBJECTIVE      = "time"     # "wait" | "time" | "distance"
START_AT_ZERO  = False
...
# (Opsiyonel) ε-kısıtlı Pareto: toplam seyir zamanı üst sınırı (dakika)
ENABLE_TIME_BUDGET = False
TIME_BUDGET = 999999  # dk (gerekirse değiştir)
...
# İsteğe bağlı: rota içindeki tüm ta/ts/td değerlerini de vardiya içine kilitle
...
# ---------- S2'de 3. aracı kapatma kuralı (isteniyorsa) ----------
S2_ALLOWED = set(K[:2])
for r in S2_ROUTES:
    for k in K:
