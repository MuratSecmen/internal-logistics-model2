...
# =====================================================================
# PARAMETRELERİ TANIMLA
# =====================================================================
TIME_LIMIT = 1200
MIP_GAP    = 0.03
THREADS    = 6
EPS_ARRIVAL = 9999  # Toplam varış zamanları için üst sınır
...
print(f"M={M}, U={U}, ε={epsilon}, EPS_ARRIVAL={EPS_ARRIVAL}, TIME_THRESHOLD={TIME_THRESHOLD}")
print("="*80 + "\n")

# =====================================================================
# MODEL OLUŞTURMA
# =====================================================================
m = gp.Model("InternalLogistics_MinWait_Primary")
...
# AMAÇ FONKSİYONU
# =====================================================================
# ✅ BİRİNCİL AMAÇ FONKSİYONU: Toplam bekleme süresini minimize et
obj1 = quicksum(w[p] for p in P)
m.setObjective(obj1, GRB.MINIMIZE)

# ✅ İKİNCİL AMAÇ (EPSILON-KISIT): Toplam varış zamanları üst sınıra tabi
obj2 = quicksum(ta['h', k, r] for k in K for r in R)
m.addConstr(obj2 <= EPS_ARRIVAL, name="eps_arrival_constraint")

print("="*80)
print("MODEL 1: w_p BİRİNCİL AMAÇ")
print("="*80)
print(f"Birincil Amaç (minimize): Σ_p w_p (Toplam Bekleme Süresi)")
print(f"İkincil Amaç (kısıt):     Σ_k Σ_r ta_hkr ≤ {EPS_ARRIVAL} dakika")
...
excel_path = os.path.join('results', f"result_wp_primary_{timestamp}.xlsx")
log_path   = os.path.join(desktop_dir, f"result_wp_primary_{timestamp}.txt")
...
if m.status in (GRB.OPTIMAL, GRB.TIME_LIMIT, GRB.SUBOPTIMAL):
    print("\n" + "="*80)
    print("ÇÖZÜM BULUNDU - MODEL 1 (w_p BİRİNCİL)")
    print("="*80)
    ...
    opt_results = pd.DataFrame([{
        ...
        'epsilon_arrival_upper': EPS_ARRIVAL,
        'arrival_slack_remaining': round(EPS_ARRIVAL - total_arrival_times, 2),
    ...
    print(f"\n{'='*80}")
    print(f"✅ Toplam bekleme süresi (minimize): {total_wait:.2f} dk")
    print(f"✅ Toplam varış zamanları (kısıtlı): {total_arrival_times:.2f} dk (üst sınır: {EPS_ARRIVAL})")
    print(f"✅ Kalan varış zamanı payı: {EPS_ARRIVAL - total_arrival_times:.2f} dk")
    print(f"{'='*80}\n")
    ...
    print("="*80)
    m.computeIIS()
    iis_file = f"infeasible_wp_primary_{timestamp}.ilp"
    m.write(iis_file)
